...
@interface M1_CasesViewController ()
{
    UIView *seachView;
    UIScrollView *typeSV;
...
@implementation M1_CasesViewController
...
- (void)seachViewHidden:(BOOL)isHidden
{
    [UIView animateWithDuration:KMiddleDuration animations:^{
        if ( isHidden) {
            ...
            seachView.frame = RectMake2x(40, 40, 1937, 1496);
        }
    }];
...
- (void)loadView{
    ...
    [[ImageView share] addToView:self.view
                   imagePathName:@"平面作品-背景" rect:RectMake2x(40, 80, 1968, 1416)];
    ...
    tb = [[UITableView alloc] initWithFrame:RectMake2x(40+512+1024, 80, 1858, 1416)];
...
- (void)openProductDetail:(UIButton *)button
{
    ...
    [self.owner.view  addSubview:lvc.view];
    [self.owner addChildViewController:lvc];
